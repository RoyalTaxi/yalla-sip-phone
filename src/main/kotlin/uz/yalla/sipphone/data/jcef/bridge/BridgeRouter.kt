...
class BridgeRouter(
    ...
    private fun logCommand(command: String, params: Map<String, String>, result: String) {
        logger.debug { "BRIDGE CMD: $command(${PhoneNumberMasker.maskParams(params)}) → $result" }
    }
    ...
    fun install(client: CefClient) {
        ...
        val router = CefMessageRouter.create(config).apply {
            addHandler(RouterHandler(), false)
        }
            ...
            val job = scope.launch {
                ...
                try {
                    ...
                    if (cmd.command != Commands.READY && !security.checkRateLimit(cmd.command)) {
                        logCommand(cmd.command, cmd.params, "RATE_LIMITED")
                    ...
                    logCommand(cmdName, emptyMap(), "INTERNAL_ERROR")
